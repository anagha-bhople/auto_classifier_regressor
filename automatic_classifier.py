...
def neural_network_classification_multiclass(x_train, x_test, y_train, y_test, D, num):
    ...
    precision = metrics.precision_score(Y_val_class, Y_pred_class, average = 'macro')
...
def neural_network_regression(x_train, x_test, y_train, y_test, loss, act_func, D, df_test, df_train):
    ...
    adjusted_r_squared =1 - (1 - r_square) * (len(df_test) - 1) / (len(df_test) - df_train.shape[1] - 1)
...
def classification_report_generation(df, target, n):
    ...
    D = {'Classifier_name':[],'Accuracy':[],'F1_score':[],'Precision':[],'Recall':[],'Time_taken':[]}
    ...
    if n==2:
        ...
        Multiclass_Classification(x_train, x_test, y_train, y_test, LogisticRegression(), "Logistic Regression",D) 
        Multiclass_Classification(x_train, x_test, y_train, y_test, GaussianNB(), "GaussianNB",D) 
        Multiclass_Classification(x_train, x_test, y_train, y_test, DecisionTreeClassifier(), "Decision Tree Classifier",D) 
        Multiclass_Classification(x_train, x_test, y_train, y_test, RandomForestClassifier(), "Random Forest Classifier",D) 
        Multiclass_Classification(x_train, x_test, y_train, y_test, GradientBoostingClassifier(), "Gradient Boosting Classifier",D) 
        Multiclass_Classification(x_train, x_test, y_train, y_test, XGBClassifier(), "XGBoost Classifier",D) 
        Multiclass_Classification(x_train, x_test, y_train, y_test, LGBMClassifier(),"Light GBM Classifier",D)
    ...
    if n>2:
        #check the evaluation metric with different classifiers out of that xgboost is performing well 
        Multiclass_Classification(x_train, x_test, y_train, y_test, LogisticRegression(multi_class='multinomial', max_iter=10000,  solver='lbfgs'), "Logistic Regression", D) 
        Multiclass_Classification(x_train, x_test, y_train, y_test, GaussianNB(), "GaussianNB",D) 
        Multiclass_Classification(x_train, x_test, y_train, y_test, DecisionTreeClassifier(), "Decision Tree Classifier",D) 
        Multiclass_Classification(x_train, x_test, y_train, y_test, RandomForestClassifier(), "Random Forest Classifier",D) 
        Multiclass_Classification(x_train, x_test, y_train, y_test, GradientBoostingClassifier(), "Gradient Boosting Classifier",D) 
...
def regression_report_generation(df, target):
    D = {'Regressor_name':[],'MSE':[],'RMSE':[],'MAE':[],'R-Square':[],'Adjusted-R-Square':[],'Time_taken':[]}
    # Selecting the columns and dividing data into train and test
    df_train=df[[col for col in list(df.columns) if col!=target]]
    df_test=df[target]
    x_train, x_test, y_train, y_test= train_test_split(df_train, df_test, test_size= 0.20, random_state=0)  
    Regression(x_train, x_test, y_train, y_test, LinearRegression(), "Linear Regressor", D, df_test, df_train) 
    Regression(x_train, x_test, y_train, y_test, DecisionTreeRegressor(), "Decision Tree Regressor", D, df_test, df_train) 
    Regression(x_train, x_test, y_train, y_test, RandomForestRegressor(), "Random Forest Regressor", D, df_test, df_train) 
    Regression(x_train, x_test, y_train, y_test, SVR(), "Support Vector regressor", D, df_test, df_train) 
    Regression(x_train, x_test, y_train, y_test, LGBMRegressor(), "Light GBM Regressor", D, df_test, df_train) 
